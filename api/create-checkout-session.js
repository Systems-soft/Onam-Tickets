 ...
 * 
 * This is a Vercel-compatible wrapper around the existing checkout logic.
 * Vercel automatically handles JSON parsing and error responses.
 */

const stripe = require('../lib/stripe');
const { getTicket, isValidTicketId } = require('../lib/tickets');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // On Vercel, use VERCEL_URL or SITE_URL environment variable
    const origin =
    ...
    if (err.message.includes('API key') || err.message.includes('authentication')) {
      return res.status(500).json({
        error: 'Stripe API key not configured. Please check your environment variables and set STRIPE_SECRET_KEY.',
      });
    }
